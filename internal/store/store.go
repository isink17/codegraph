...
// VectorSearch performs brute-force cosine similarity search over symbol embeddings.
func (s *Store) VectorSearch(ctx context.Context, repoID int64, queryVec []float32, limit, offset int) ([]map[string]any, error) {
	...
	end := min(offsetVal+limitVal, len(candidates))
	if offsetVal >= len(candidates) {
		return nil, nil
	}
	candidates = candidates[offsetVal:end]
